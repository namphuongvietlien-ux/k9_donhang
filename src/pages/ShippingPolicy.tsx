...
const generateDescription = (
  ...
  return "Chính sách vận chuyển và giao hàng. Thông tin về phí vận chuyển, thời gian giao hàng và khu vực giao hàng.";
};

const ShippingPolicy = () => {
  const { data: pageContent, isLoading } = useQuery({
    queryKey: ["page-content", "shipping-policy"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("page_contents")
        .select("*")
        .eq("page_key", "shipping-policy")
        .maybeSingle();
  ...
  const pageTitle = pageContent?.title || "Chính sách vận chuyển";
  const description = generateDescription(pageContent?.subtitle, content.sections);
  const keywords = "chính sách vận chuyển, shipping policy, giao hàng, phí vận chuyển, thời gian giao hàng";
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">
                {pageContent?.title || "Chính sách vận chuyển"}
              </h1>
