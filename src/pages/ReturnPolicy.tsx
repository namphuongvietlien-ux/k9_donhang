...
const generateDescription = (
  ...
  return "Chính sách đổi trả và hoàn tiền. Quy định về việc đổi trả sản phẩm và điều kiện áp dụng.";
};

const ReturnPolicy = () => {
  const { data: pageContent, isLoading } = useQuery({
    queryKey: ["page-content", "return-policy"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("page_contents")
        .select("*")
        .eq("page_key", "return-policy")
        .maybeSingle();
  ...
  const pageTitle = pageContent?.title || "Chính sách đổi trả";
  const description = generateDescription(pageContent?.subtitle, content.sections);
  const keywords = "chính sách đổi trả, return policy, hoàn tiền, đổi hàng, chính sách hoàn trả";
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl md:text-5xl font-serif font-bold mb-4">
                {pageContent?.title || "Chính sách đổi trả"}
              </h1>
