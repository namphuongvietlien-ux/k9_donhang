-- =====================================================
-- Tồn Q7 / products không được âm
-- Chạy trên Supabase SQL Editor (production)
-- =====================================================

-- 1) Sửa dữ liệu đang âm
...
-- 2) Ràng buộc products.stock_quantity >= 0
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'products_stock_quantity_nonneg'
  ) THEN
...
EXCEPTION
  WHEN others THEN
    RAISE NOTICE 'Không gắn CHECK products: %', SQLERRM;
END $$;

-- 3) Đảm bảo stock_on_hand vẫn có CHECK >= 0
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'stock_on_hand_quantity_check'
       OR conname LIKE '%stock_on_hand%quantity%'
  ) THEN
    ALTER TABLE public.stock_on_hand
      ADD CONSTRAINT stock_on_hand_quantity_nonneg
      CHECK (quantity >= 0);
  END IF;
EXCEPTION
  WHEN others THEN
    RAISE NOTICE 'Không gắn CHECK stock_on_hand: %', SQLERRM;
END $$;

-- 4) Trigger kẹp >= 0 cho mọi đường ghi (FIFO / ecommerce / app)
...
-- 5) Ecommerce: trừ tồn không cho âm
...
COMMENT ON FUNCTION public.deduct_stock_on_ecommerce_delivery(UUID) IS
  'Trừ tồn ecommerce — stock_quantity không xuống dưới 0';
